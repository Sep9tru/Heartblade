...
public class Superme implements ISpecialEffect {
    static final String EffectKey = "Superme";
    ...
    public void doAddAttack(ItemStack stack, EntityPlayer player, ItemSlashBlade.ComboSequence setCombo) {
        ...
        if (!world.isRemote) {
            player.addPotionEffect(new PotionEffect(10, 200, 0));
                            player.addPotionEffect(new PotionEffect(22, 20, 1));
                            player.addPotionEffect(new PotionEffect(21, 20, 1));
            int i;
            for (i = 0; i <= 8; ++i) {
                ItemStack ggg = player.inventory.getStackInSlot(i);
                if (ggg != null && (ggg.getItem() == CommonProxy.ziminblade)) {
