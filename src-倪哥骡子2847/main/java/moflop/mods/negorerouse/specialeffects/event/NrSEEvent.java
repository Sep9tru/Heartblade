...
//    永恒之枪即死
    ...
    public void InstKillEffectEvent(SlashBladeEvent.ImpactEffectEvent event){
        ...
        if (!event.blade.getDisplayName().equals(BladeUtils.findItemStack(NegoreRouse.MODID,"moflop.slashblade.chronos_sy",1).getDisplayName())) return;
...
//        if(!useBlade(event.sequence)) return;
        ...
        ItemStack blade = event.blade;
        if (!(blade.getItem() instanceof ItemNrSlashBlade)) return;
        if (!event.blade.getDisplayName().equals(BladeUtils.findItemStack(NegoreRouse.MODID,"moflop.slashblade.artemis_s",1).getDisplayName())) return;
        EntityPlayer player = (EntityPlayer) event.user;
...
//        if(!useBlade(event.sequence)) return;
        ...
        ItemStack blade = event.blade;
        if (!(blade.getItem() instanceof ItemNrSlashBlade)) return;
        if (!event.blade.getDisplayName().equals(BladeUtils.findItemStack(NegoreRouse.MODID,"moflop.slashblade.tartarus",1).getDisplayName())) return;
        EntityPlayer player = (EntityPlayer) event.user;
        EntityLivingBase target = event.target;
    ...
    public void FateEffectEvent(SlashBladeEvent.ImpactEffectEvent event) {
        ...
        ItemStack blade = event.blade;
        if (!(blade.getItem() instanceof ItemNrSlashBlade)) return;
        if (!event.blade.getDisplayName().equals(BladeUtils.findItemStack(NegoreRouse.MODID,"moflop.slashblade.ananke",1).getDisplayName())) return;
        EntityPlayer player = (EntityPlayer) event.user;
        EntityLivingBase target = event.target;
        World world = player.world;
    ...
    public void PhantomEffectEvent(SlashBladeEvent.OnUpdateEvent event) {
        ...
        ItemStack blade = event.blade;
        if (!(blade.getItem() instanceof ItemNrSlashBlade)) return;
        if (!event.blade.getDisplayName().equals(BladeUtils.findItemStack(NegoreRouse.MODID,"moflop.slashblade.nyx",1).getDisplayName())) return;
...
//        if (!useBlade(event.sequence)) return;
        ...
        ItemStack blade = event.blade;
        if (!(blade.getItem() instanceof ItemNrSlashBlade)) return;
        if (!event.blade.getDisplayName().equals(BladeUtils.findItemStack(NegoreRouse.MODID,"moflop.slashblade.chronos_ae",1).getDisplayName())) return;
        EntityPlayer player = (EntityPlayer) event.user;
        EntityLivingBase target = event.target;
        World world = player.world;
