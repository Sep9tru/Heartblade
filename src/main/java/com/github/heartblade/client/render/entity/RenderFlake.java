...
public class RenderFlake extends Render
{
    private static double[][] dVec;
    private static int[][] nVecPos;

    public RenderFlake(final RenderManager renderManager) {
        super(renderManager);
    }

    public void doRender(final Entity entity, final double d0, final double d1, final double d2, final float f, final float f1) {
        if (entity instanceof EntityFlake) {
            this.doDriveRender((EntityFlake)entity, d0, d1, d2, f, f1);
        }
    }



    private void doDriveRender(final EntityFlake entitySummonedBlade, final double dX, final double dY, final double dZ, final float f, final float f1) {
        final Tessellator tessellator = Tessellator.getInstance();
        GL11.glDisable(3553);
        GL11.glDisable(2896);
        GL11.glEnable(3042);
        int color = entitySummonedBlade.getColor();
        final boolean inverse = color < 0;
        ...
        final float time = (entitySummonedBlade.hitTime != 0L) ? (entitySummonedBlade.hitTime % 6L + entitySummonedBlade.hitStopFactor) : (entitySummonedBlade.getEntityWorld().getWorldTime() % 6L + f1);
        GL11.glRotatef(time * 60.0f, 0.0f, 1.0f, 0.0f);
        final float scale = 0.01f;
        GL11.glScalef(scale, scale, scale);
        final float lifetime = entitySummonedBlade.getLifeTime();
        final float ticks = entitySummonedBlade.ticksExisted;
        final BufferBuilder wr = tessellator.getBuffer();
        wr.begin(4, DefaultVertexFormats.POSITION_COLOR);
        final int r = color >> 16 & 0xFF;
        final int g = color >> 8 & 0xFF;
        final int b = color & 0xFF;
        final double dScale = 0.25;
        for (int idx = 0; idx < RenderFlake.nVecPos.length; ++idx) {
            wr.pos(RenderFlake.dVec[RenderFlake.nVecPos[idx][0]][0] * dScale, RenderFlake.dVec[RenderFlake.nVecPos[idx][0]][1] * dScale, RenderFlake.dVec[RenderFlake.nVecPos[idx][0]][2] * dScale).color(r, g, b, 255).endVertex();
            wr.pos(RenderFlake.dVec[RenderFlake.nVecPos[idx][1]][0] * dScale, RenderFlake.dVec[RenderFlake.nVecPos[idx][1]][1] * dScale, RenderFlake.dVec[RenderFlake.nVecPos[idx][1]][2] * dScale).color(r, g, b, 255).endVertex();
            wr.pos(RenderFlake.dVec[RenderFlake.nVecPos[idx][2]][0] * dScale, RenderFlake.dVec[RenderFlake.nVecPos[idx][2]][1] * dScale, RenderFlake.dVec[RenderFlake.nVecPos[idx][2]][2] * dScale).color(r, g, b, 255).endVertex();
        }
        tessellator.draw();
        GL11.glPopMatrix();
        GL11.glDisable(3042);
    ...
    protected /* bridge */ ResourceLocation getEntityTexture(final Entity var1) {
        return this.getEntityTexture(var1);
    }

    static {
        RenderFlake.dVec = new double[][] { { -29.5747D, 0.0D, -209.4692D }, { -63.8415D, 0.0D, -182.0565D }, { 26.9376D, 0.0D, -188.8279D }, { 6.3064D, 0.0D, -278.8316D }, { 34.3655D, 0.0D, -93.2845D }, { 76.1149D, 0.0D, -200.8026D }, { -4.1153D, 0.0D, -132.5763D }, { 128.5629D, 0.0D, -148.7683D }, { 137.2516D, 0.0D, -101.4414D }, { 165.5407D, 0.0D, -139.2845D },
        ...
        RenderFlake.nVecPos = new int[][] { { 37, 32, 33 }, { 37, 23, 32 }, { 32, 35, 29 }, { 23, 116, 32 }, { 18, 27, 23 }, { 27, 24, 23 }, { 18, 26, 27 }, { 18, 168, 167 }, { 16, 21, 167 }, { 16, 20, 21 },
