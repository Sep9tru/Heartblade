 ...
 */
public class EntitySlashDimensionP_core extends Entity implements IThrowableEntity {
    /**
     * ★撃った人
     */
     ...
     * @param par1World
     */
    public EntitySlashDimensionP_core(World par1World)
    {
        super(par1World);
        ticksExisted = 0;
    ...
    public EntitySlashDimensionP_core(World par1World, EntityLivingBase entityLiving, float AttackLevel, boolean multiHit){
        this(par1World, entityLiving, AttackLevel);
        this.setIsSingleHit(multiHit);
    }

    public EntitySlashDimensionP_core(World par1World, EntityLivingBase entityLiving, float AttackLevel)
    {
        this(par1World);
    ...
    private static final DataParameter<Integer> LIFETIME = EntityDataManager.<Integer>createKey(EntitySlashDimensionP_core.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> SINGLE_HIT = EntityDataManager.<Boolean>createKey(EntitySlashDimensionP_core.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> IS_SLASH_DIMENSION = EntityDataManager.<Boolean>createKey(EntitySlashDimensionP_core.class, DataSerializers.BOOLEAN);

    private static final DataParameter<Integer> THROWER_ENTITY_ID = EntityDataManager.<Integer>createKey(EntitySlashDimensionP_core.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> INTERVAL = EntityDataManager.<Integer>createKey(EntitySlashDimensionP_core.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> COLOR = EntityDataManager.<Integer>createKey(EntitySlashDimensionP_core.class, DataSerializers.VARINT);
    ...
    public void onUpdate()
    {
        ...
        if(!world.isRemote)
        {
            ...
            if(this.getThrower() != null){
                ...
                if(getIsSingleHit() || this.ticksExisted % 1 == 0){
                            ...
                            if(!blade.isEmpty() && curEntity instanceof EntityLivingBase) {
                                if(getIsSlashDimension()){
                                    curEntity.addVelocity(
                                            (double) (Math.sin(getThrower().rotationYaw * (float) Math.PI / 180.0F) * -0.1),
                                            0.1,
                                            (double) (-Math.cos(getThrower().rotationYaw * (float) Math.PI / 180.0F) * -0.1));
