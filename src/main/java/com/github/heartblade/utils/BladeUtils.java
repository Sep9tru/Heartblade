...
public class BladeUtils {
    public static Map<ResourceLocationRaw, ItemStack> hbBladeRegistry = Maps.newHashMap();
    public static List<String> HbNamedBlades = Lists.newArrayList();

    public static void registerCustomItemStack(String name, ItemStack stack){
        hbBladeRegistry.put(new ResourceLocationRaw(heartblade.MODID, name),stack);
    }
    ...
    static public ItemStack findItemStack(String modid, String name, int count){
        ...
        if(hbBladeRegistry.containsKey(key)) {
            stack = hbBladeRegistry.get(key).copy();
