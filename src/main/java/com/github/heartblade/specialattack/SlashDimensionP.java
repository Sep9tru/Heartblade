...
import com.github.heartblade.entity.EntitySlashDimensionP_core;
 ...
 */
public class SlashDimensionP extends SpecialAttackBase implements IJustSpecialAttack, ISuperSpecialAttack {
    @Override
    public String toString() {
        return "slashdimensionp";
    }
    ...
    public void doSpacialAttack(ItemStack stack, EntityPlayer player) {
        ...
        if(target == null) {
            ...
            if(!player.world.isRemote) {
                ...
                EntitySlashDimensionP_core dim = new EntitySlashDimensionP_core(world, player, magicDamage);
                if (dim != null) {
            ...
            final int cost = -25;
            if(!ItemSlashBlade.ProudSoul.tryAdd(tag, cost, false)){
                ItemSlashBlade.damageItem(stack, 2, player);
            }
    ...
    public void doJustSpacialAttack(ItemStack stack, EntityPlayer player) {
        ...
        if(target == null) {
            ...
            player.playSound(SoundEvents.ENTITY_ENDERMEN_TELEPORT, 1.0F, 1.5F);
            player.playSound(SoundEvents.ENTITY_BLAZE_HURT, 1.0F, 2.0F);
            ...
            if(!player.world.isRemote) {
                ...
                EntitySlashDimensionP dim = new EntitySlashDimensionP(world, player, magicDamage);
                if (dim != null) {
                    dim.setPosition(pos.x, pos.y, pos.z);
                    dim.setLifeTime(20);
                    dim.setIsSlashDimension(true);
                    dim.setIsSingleHit(true);
                    world.spawnEntity(dim);
                }
                EntitySlashDimensionP_core dimc = new EntitySlashDimensionP_core(world, player, magicDamage);
                if (dimc != null) {
                    dimc.setPosition(pos.x, pos.y, pos.z);
                    dimc.setLifeTime(25);
                    dimc.setIsSlashDimension(true);
                    dim.setIsSingleHit(true);
                    world.spawnEntity(dimc);
                }
            }
            ...
            int level = EnchantmentHelper.getEnchantmentLevel(Enchantments.POWER, stack);
            float magicDamage = ItemSlashBlade.AttackAmplifier.get(tag) * (level / 2.0f);
                for(int i = 0; i < 5;i++){

                    EntityDriveEx entityDrive = new EntityDriveEx(world, player, Math.min(5.0f,magicDamage/2.0f));
...
//                        entityDrive.setIsSlashDimension(type.contains(ItemSlashBlade.SwordType.FiercerEdge));
            ...
            if(!target.world.isRemote){
                ...
                EntitySlashDimensionP_core dimc = new EntitySlashDimensionP_core(world, player, magicDamage);
                if (dimc != null) {
                    dimc.setPosition(target.posX,target.posY + target.height / 2.0,target.posZ);
                    dimc.setLifeTime(25);
                    dimc.setIsSlashDimension(true);
                    dim.setIsSingleHit(false);
                    world.spawnEntity(dimc);
