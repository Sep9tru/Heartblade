...
public class ToRavage implements ISpecialEffect {
    static final String EffectKey = "ToRavage";
    ...
    public void onImpactEffectEvent(SlashBladeEvent.ImpactEffectEvent event) {
        if (this.useBlade(event.sequence)) {
            if (SpecialEffects.isPlayer(event.user)) {
                ...
                event.target.addPotionEffect(new PotionEffect(Potion.weakness.getId(), 100, 6));
    ...
    public void doAddAttack(ItemStack stack, EntityPlayer player, ItemSlashBlade.ComboSequence setCombo) {
        ...
        if (!world.isRemote) {
            player.addPotionEffect(new PotionEffect(Potion.invisibility.getId(), 20, 0));
            int i;
            for (i = 0; i <= 8; ++i) {
                ItemStack aaa = player.inventory.getStackInSlot(i);
                if (aaa != null && (aaa.getItem() == CommonProxy.matongcblade)) {
