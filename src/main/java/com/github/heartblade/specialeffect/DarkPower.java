...
public class DarkPower implements ISpecialEffect {
    static final String EffectKey = "DarkPower";
    ...
    public void onImpactEffectEvent(SlashBladeEvent.ImpactEffectEvent event) {
        if (this.useBlade(event.sequence)) {
            if (SpecialEffects.isPlayer(event.user)) {
                ...
                event.target.clearActivePotions();
    ...
    public void doAddAttack(ItemStack stack, EntityPlayer player) {
        World world = player.worldObj;
        NBTTagCompound tag = ItemSlashBlade.getItemTagCompound(stack);
        player.clearActivePotions();
        if (!world.isRemote) {
