...
public class Breakeverything implements ISpecialEffect {
    static final String EffectKey = "Breakeverything";
    protected int Level = 1;
    public Breakeverything() {
    }
    ...
    public void doAddAttack(ItemStack stack, EntityPlayer player, ItemSlashBlade.ComboSequence setCombo) {
        ...
        if (!world.isRemote) {
            player.addPotionEffect(new PotionEffect(10, 20, 1));
            player.addPotionEffect(new PotionEffect(22, 20, 1));
            player.addPotionEffect(new PotionEffect(16, 20, 0));
            player.addPotionEffect(new PotionEffect(13, 200, 0));
                int i;
                for (i = 0; i <= 8; ++i) {
                    ItemStack ggg = player.inventory.getStackInSlot(i);
                    if (ggg != null && (ggg.getItem() == CommonProxy.xiaochaoblade)) {
