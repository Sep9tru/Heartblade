...
public class SolemnLament implements ISpecialEffect {

    private static final String EffectKey = "SolemnLament";
    ...
    public void onImpactEffectEvent(SlashBladeEvent.ImpactEffectEvent event) {
        if (this.useBlade(event.sequence)) {
            if (SpecialEffects.isPlayer(event.user)) {
                ...
                event.target.addPotionEffect(new PotionEffect(7, 20, 1));
                event.target.addPotionEffect(new PotionEffect(20, 60, 1));
