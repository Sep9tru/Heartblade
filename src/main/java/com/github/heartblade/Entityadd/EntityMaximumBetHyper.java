...
public class EntityMaximumBetHyper extends Entity implements IThrowableEntity {
    ...
    public EntityMaximumBetHyper(World par1World) {
        super(par1World);
        this.ticksExisted = 0;
    }
    ...
    public EntityMaximumBetHyper(World par1World, EntityLivingBase entityLiving) {
        this(par1World);
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted == 1 && getThrower() != null) {
            ...
            if (getThrower() != null && getThrower() instanceof EntityPlayer) {
                EntityPlayer player = (EntityPlayer)getThrower();
                player.playSound("heartblade:don06", 0.6F, 1.2F + player.getRNG().nextFloat() * 0.05F);
            }
    ...
    public void doAttack(ItemSlashBlade.ComboSequence combo) {
        ...
        if (this.blade != null) {
            ...
            for (Entity curEntity : list) {
                ...
                curEntity.attackEntityFrom((DamageSource)entityDamageSource, 30.0F);
        ...
        if (getThrower() != null) {
            ...
            magicDamage = baseModif + 30;
            if (5 <= rank)
                magicDamage += ItemSlashBlade.AttackAmplifier.get(tag).floatValue() * (42.0F + level / 1.0F);
        }
        EntityDriveAdd entityDrive = new EntityDriveAdd(this.worldObj, entityLiving, magicDamage, true, 90.0F - Math.abs(combo.swingDirection));
        if (entityDrive != null) {
            entityDrive.setColor(7396315);
            entityDrive.setInitialSpeed(1.5F);
            entityDrive.setScale(3.0F);
